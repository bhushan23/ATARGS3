...
class ReceptionController {
	...
	def index() { 
		...
		[limits: [prestart, start, last],  free: freeslots ,	result:result]
	...
	def done(){							//action for saving sms changes
		...
		smsEntry.medicalHistory=params["history"]
		smsEntry.save()
...
def ShowApp(){
	...
	def myPatients=db.rows("Select * from patient_details where confirmed_date REGEXP ${queryDate}")
	render(view:"ScanMyAppointments",model:[result:myPatients])
}
 ...
 */
 ...
 class ReceptionController {
	 ...
	 def rejectApp() {
		 ...
		 if(AdminSettingsController.smsservice){
			 print 'sending sms '
		 SendsmsController.smslane_single_no("91"+pd.mobile,"Sorry,your appointment for ${pd.machine} scan has been conceled.Please call the customer care.")
		 pd.delete();
		 } 
		 redirect(action:"getNewApp");
		 //send sms that patient has to get appointment again
