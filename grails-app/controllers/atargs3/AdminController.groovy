...
class AdminController {
	...
	def confirmReceptionAcc(){
		def temp=Reception.get(params.get("p"))
		temp.confirmedFlag=1
		temp.save()
	...
	def confirmDoctorAcc(){
		def temp=Doctor.get(params.get("p"))
		temp.confirmedFlag=1
		temp.save()
	...
	def viaSMS(){
		...
		def result=db.rows("Select * from patient_details Where appointmenttype = '1' AND confirmed_date >= ${lower} AND confirmed_date <= ${upper}")
		[appViaSms:result]
	}
	def smsEntry
	int uid
